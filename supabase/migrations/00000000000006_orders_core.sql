-- =============================================
-- Etapa 8 — Pedidos: token de seguimiento, historial de estados
-- y creación transaccional vía RPC.
-- =============================================

create extension if not exists pgcrypto with schema extensions;

-- Token corto y único para el enlace de seguimiento público
alter table orders
  add column tracking_token text not null unique
  default upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 10));

-- ---------- Historial de estados del pedido ----------
-- Quién y cuándo cambió cada estado (sección 54 de la especificación)
...
-- Todo o nada: cliente, dirección, pedido, ítems (snapshot) y pago.
...
declare
  ...
  v_item_count int := 0;
begin
  -- 1. El negocio debe estar abierto (hora local de la base, America/Lima)
  ...
  if not (
    (v_settings.open_time <= v_settings.close_time
      and localtime >= v_settings.open_time
      and localtime <  v_settings.close_time)
    or
    (v_settings.open_time > v_settings.close_time
      and (localtime >= v_settings.open_time or localtime < v_settings.close_time))
  ) then
    raise exception 'NEGOCIO_CERRADO';
  end if;
  ...
  -- 6. Pedido (order_number se autogenera; número y fecha nunca cambian)
